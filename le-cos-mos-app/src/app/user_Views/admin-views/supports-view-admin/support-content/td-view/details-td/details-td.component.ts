...
export class DetailsTdComponent implements OnInit ,AfterViewInit {
...
cour_name?:string = this.route.url.split(/\//g)[this.route.url.split(/\//g).length - 4]
downloadFile() {
  const querParam = new HttpParams().set('filename', this.documentTitle!).set('contentType',this.contentType).set('cour_name',this.cour_name!)
